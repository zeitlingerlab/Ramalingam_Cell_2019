...
options(knitr.figure_dir = "figure1b_enrichments_diff_thres")
...
``` {r samples_table}
samples.df <- read.csv("/lola_paper/aws/analysis/data/figure1b_polII_data.csv", stringsAsFactors=FALSE)
samples.df %>% pander
```
...
enrichments.df %<>% group_by(label, factor) %>%
            mutate(normalized_enrichment = pmax(0, pmin(1, enrichment / quantile(enrichment, 0.80))))
