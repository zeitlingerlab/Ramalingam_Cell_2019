...
namespace Tools
{
    ...
    public static class Stats
    {
        ...
        *************************************************************************/
        ...
        /// <summary>
        /// The name of the spearman.
        /// </summary>
        private static string SpearmanName = "spearman";

        /// <summary>
        /// The name of the pearson.
        /// </summary>
        private static string PearsonName = "pearson";

        /// <summary>
        /// Spearman the specified data, column1 and column2.
        /// </summary>
        /// <param name="data">Data.</param>
        /// <param name="column1">Column1.</param>
        /// <param name="column2">Column2.</param>
        public static CorrelationData Spearman(IEnumerable<IEnumerable<double>> data, string column1, string column2)
        {
            return Correlation(data.Select(x => x.Select(y => y.ToString())), column1, column2, SpearmanName);
        }
        ...
        /// <param name="column1">Column1.</param>
        /// <param name="column2">Column2.</param>
        public static CorrelationData Spearman(IEnumerable<IEnumerable<string>> data, string column1, string column2)
        {
            return Correlation(data, column1, column2, SpearmanName);
        }

        /// <summary>
        /// Pearson the specified data, column1 and column2.
        /// </summary>
        /// <param name="data">Data.</param>
        /// <param name="column1">Column1.</param>
        /// <param name="column2">Column2.</param>
        public static CorrelationData Pearson(IEnumerable<IEnumerable<string>> data, string column1, string column2)
        {
            return Correlation(data, column1, column2, PearsonName);
        ...
        /// <param name="column1">Column1.</param>
        /// <param name="column2">Column2.</param>
        public static CorrelationData Pearson(IEnumerable<IEnumerable<double>> data, string column1, string column2)
        {
            return Correlation(data.Select(x => x.Select(y => y.ToString())), column1, column2, PearsonName);
        }

        /// <summary>
        /// Correlation the specified data, column1, column2 and type.
        /// </summary>
        /// <param name="data">Data.</param>
        /// <param name="column1">Column1.</param>
        /// <param name="column2">Column2.</param>
        /// <param name="type">Type.</param>
        private static CorrelationData Correlation(IEnumerable<IEnumerable<string>> data, string column1, string column2, string type)
        {
            ...
            string scriptFile = dataFile + ".R";
            R.WriteAndRunScript(
                new string[]
            {
                R.ReadTable(R.Variables.X, tableFile, true),
                string.Format("corrdata = c(cor(x=x${0}, y=x${1}, method='{2}'), cor.test(x=x${0}, y=x${1}, method='{2}')$p.value)", column1, column2, type),
                R.WriteTable("corrdata", dataFile)
            },
                scriptFile);

            var corrData = Helpers.GetFileDataLines(dataFile, true).Select(x => x.Split(' ')[1]).ToArray();

            return new CorrelationData
            {
                R = double.Parse(corrData[0]),
                Pvalue = double.Parse(corrData[1]),
            };
        }

        public static double BinomTest(int successes, int total, double probability = 0.5)
        {
            string tableFile = Path.GetTempFileName();
            Tables.ToNamedTsvFile(
                tableFile,
                new string[][] { new string[] { successes.ToString(), total.ToString(), probability.ToString() } },
                new string[] { "S", "T", "P" });

            string dataFile = ".dat";//Path.GetTempFileName();
            string scriptFile = dataFile + ".R";
            R.WriteAndRunScript(
                new string[]
            {
                R.ReadTable(R.Variables.X, tableFile, true),
                "p = c(binom.test(x$S[1], x$T[1], x$P[1]))$p.value",
                R.WriteTable("p", dataFile)
            },
                scriptFile);

            var corrData = Helpers.GetFileDataLines(dataFile, true).Select(x => x.Split(' ')[1]).ToArray();

            return double.Parse(corrData[0]);
        }
        ...
        public static double Spearman(IEnumerable<Point2> vector)
        {
            var xRanks = vector
                .OrderBy(x => x.x)
                .Select((x, i) => new { X = x.x, Rank = i })
                .ToLookup(x => x.X, x => x.Rank)
                .ToDictionary(x => x.Key, x => x.Average());

            var yRanks = vector
                .OrderBy(x => x.y)
                .Select((x, i) => new { Y = x.y, Rank = i })
                .ToLookup(x => x.Y, x => x.Rank)
                .ToDictionary(x => x.Key, x => x.Average());

            return Pearson(vector.Select(x => new Point2 { x = xRanks[x.x], y = yRanks[x.y] }));    
        }
        ...
        public static double FisherTransformZScore(double r, int n)
        {
            ...
            //if (r > 0.99)
            //{
             //   var z = 0.5 * (Math.Log(1 + r) - Math.Log(1 - r)) * Math.Sqrt(n - 3);
            //}
            return 0.5 * (Math.Log(1 + r) - Math.Log(1 - r)) * Math.Sqrt( n - 3 );
        }
        ...
        /// <summary>
        /// Calculates a median using pre-sorted data
        /// </summary>
        /// <returns>The median.</returns>
        /// <param name="data">Data.</param>
        public static double SortedMedian(List<double> data)
        {
            int count = data.Count;
            if (count == 0)
            {
