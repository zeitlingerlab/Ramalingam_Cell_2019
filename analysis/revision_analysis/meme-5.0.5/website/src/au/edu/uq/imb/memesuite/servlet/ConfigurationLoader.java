...
import au.edu.uq.imb.memesuite.db.CismapperDBList;
...
public class ConfigurationLoader implements ServletContextListener {
  ...
  public static final String CISMAPPER_DB_KEY = "au.edu.uq.imb.memesuite.cismapperdb";
...
public void contextDestroyed(ServletContextEvent sce) {
  ...
  public class CsvWatcher implements Runnable {
    ...
    private void updateCismapperDb() {
      CismapperDBList db = (CismapperDBList)context.getAttribute(CISMAPPER_DB_KEY);
      File currentTsv = (db == null ? null : db.getTSV());
      File foundTsv = findCsv(config, "cismapper_databases", "cismapper_db.tsv");
      if (foundTsv == null) return;
      if (currentTsv == null || !foundTsv.equals(currentTsv) ||
          db.lastModified() != foundTsv.lastModified()) {
        if (currentTsv != null) logger.log(Level.INFO, "Found a changed CisMapper database TSV at " + foundTsv);
        CismapperDBList db2;
        try {
           db2 = new CismapperDBList(foundTsv);
        } catch (Exception e) {
          logger.log(Level.SEVERE, "Error loading CisMapper database", e);
          return;
        }
        context.setAttribute(CISMAPPER_DB_KEY, db2);
        if (db != null) delayedCleanup.offer(db);
      }
    }
